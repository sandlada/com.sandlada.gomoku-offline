<template>
  <div class="board-frame" :class="{ thinking: game.aiThinking }">
    <div class="coords-row" aria-hidden="true">
      <span v-for="label in COL_LABELS" :key="label" class="coord col">{{ label }}</span>
    </div>
    <div class="board-body">
      <div class="coords-col" aria-hidden="true">
        <span v-for="n in size" :key="n" class="coord row">{{ n }}</span>
      </div>
      <div
        class="grid"
        role="grid"
      ...
      >
        ...
        />
        <template v-for="(row, r) in game.board" :key="r">
          <button
            v-for="(cell, c) in row"
            :key="`${r}-${c}`"
            :id="`cell-${r}-${c}`"
          ...
          >
            <span v-if="cell !== null" class="stone" :class="cell" />
            <span v-else class="ghost" :class="game.current" />
          </button>
        </template>
      </div>
    </div>
    <p v-if="pending !== null" class="confirm-hint">{{ t('game.confirm') }}</p>
